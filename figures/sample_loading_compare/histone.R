...
# ggplot(data_res) +
#     facet_wrap(~Dim, scales = "free_y", ncol = 1) +
#     geom_line(data = data_oxygen, aes(x = Time, y = dO2_scaled, color = 'Dissolved oxygen concentration', shape = 'Dissolved oxygen concentration'), linewidth = 0.25) +
#     geom_line(aes(x = Time, y = Loading, color = data), linewidth = 0.25) +
#     geom_point(aes(x = Time, y = Loading, shape = data, color = data), size = 0.5) +
#     geom_hline(yintercept = 0, linetype = "longdash", linewidth = 0.2, color = 'grey45') +
#     scale_y_continuous(sec.axis = sec_axis( ~ (. + 0.7) / scale_factor, name = "dO2(%)")) +
#     # 手动设置图例对应，来确保三条线都显示到图例中
#     scale_color_manual(values = colors, breaks = breakss) +
#     scale_shape_manual(values = shapes, breaks = breakss) +
#     labs(x = "Time(h)", y = "Loading", color = "Data", shape = "Data") +
...
ggsave("sample_loading_histone&YMC2014.pdf", width = 9, height = 8.5, units = "cm" ) # pdf会带有0.4cm左右的白边，可以适当加大尺寸
